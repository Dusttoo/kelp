...
const Questions = () => {
  ...
  const getOwner = (questionId) => {
    const question = eachQuestion.find(
      (questions) => questionId === questions.id
    );
    const user = eachUser.find(
      (oneQuestion) => question.userId === oneQuestion.id
    );
    return user;
  };
            ...
            <div className="questionSection" key={thisQuestion.id}>
              <div className="reviewer">
                <img
                  ...
                  src={`${getOwner(thisQuestion.id).profileImg}`}
                  alt={getOwner(thisQuestion.id).firstName}
                ></img>
                <div className="reviewer-info">
                  <h3 className="reviewer-name">
                    {getOwner(thisQuestion.id).firstName}{" "}
                    {getOwner(thisQuestion.id).lastName}
                  </h3>
                  <h4 className="reviewer-title">
                    Kelping Since: {getDate(getOwner(thisQuestion.id).id)}
                  </h4>
                </div>
              </div>
              <div className="q-a">
                <div className="q-tag">
                  <h3 className="question-letter">Q:</h3>
                  <p className="quesion-asked">{thisQuestion.question}</p>
                </div>
                <div className="q-tag">
                  <h3 className="question-letter">A:</h3>
                  <p className="quesion-asked">{thisQuestion.answer}</p>
                </div>
              </div>
              {sessionUser ? (
                <div className="edit-deletey">
                  {thisQuestion.userId === sessionUser.id ? (
                    <div className="edit-delete-review">
                      <Link
                        to={`/questions/${thisQuestion.id}`}
                        className="add-biz"
                      >
                        Edit{" "}
                      </Link>
                      <Link
                        to={`/questions/${thisQuestion.id}/delete`}
