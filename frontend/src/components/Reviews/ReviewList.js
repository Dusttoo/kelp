            ...
            <div key={thisReview.id} className="review">
              <div className="reviewer">
                <img
                  ...
                  src={`${getOwner(thisReview.id).profileImg}`}
                  alt={getOwner(thisReview.id).firstName}
                ></img>
                <div className="reviewer-info">
                  <h3 className="reviewer-name">
                    {getOwner(thisReview.id).firstName}{" "}
                    {getOwner(thisReview.id).lastName}
                  </h3>
                  <h4 className="reviewer-title">
                    Kelping Since: {getDate(getOwner(thisReview.id).id)}
                  </h4>
                </div>
              </div>
                ...
                <div className="edit-deletey">
                  {thisReview.userId === sessionUser.id ? (
                    <div className="edit-delete-review">
                      <Link
                        to={`/reviews/${thisReview.id}`}
                        className="add-biz"
                      >
                        Edit{" "}
                      </Link>
                      <Link
                        to={`/reviews/${thisReview.id}/delete`}
